...
public class UserLocation implements LocationListener, IObservable {
	...
	public void onLocationChanged(Location location) {
		IObserver.Status status = IObserver.Status.NORMAL_UPDATE;
		if (this.currentLocation == null) {
			this.currentLocation = location;
			status = IObserver.Status.FIRST_LOCATION;
	 ...
	 */
	public IObserver.Status getProviderStatus() {
		...
		if (!(netEnabled || gpsEnabled)) {
			return IObserver.Status.ALL_DISABLED;
		} else if (!netEnabled) {
			return IObserver.Status.NET_DISABLED;
		} else if (!gpsEnabled) {
			return IObserver.Status.GPS_DISABLED;
		}
		return IObserver.Status.NORMAL_UPDATE;
	}
